...
from src.utils.dataset import AudioEmotionDataset, MelspecEmotionDataset
from src.utils.constants import Columns, Emotions, Datasets
import src.utils.data_processing as data_processing



class AudioEmotionDatasetTestCase(TestCase):
    ...
    def test_transform_spectrogram_plot(self):
        ...
        for i in range(min(len(dataset), 5)):
            ...
            plt.tight_layout()
            # set range of x-axis to be the same as the range of the audio file
            plt.xlim([0, input.shape[2]])
            # plt.show()
            plt.pause(1)  # display each plot for 1 seconds

class MelspecEmotionDatasetTestCase(TestCase):
    def test_dataset(self):
        csv_df = data_processing.read_csv(
            pd.read_csv(
        ...
        dataset = MelspecEmotionDataset(csv_df)

        print()
        print('dataset.n_mels', dataset.n_mels)
        print('dataset.max_timeframes', dataset.max_timeframes)
        ...
        for i in range(min(len(dataset), 5)):
            ...
            plt.figure(figsize=(10, 4))
            plt.imshow(input[0].numpy(), cmap='hot', origin='lower', aspect='auto')
            plt.title('Mel Spectrogram From MelspecEmotionDataset')
            plt.colorbar(format='%+2.0f dB')
            plt.tight_layout()
            # set range of x-axis to be the same as the range of the audio file
            plt.xlim([0, input.shape[2]])
